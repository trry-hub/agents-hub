...
test('webview conversation transcript surfaces compact metadata and readable code output', () => {
  ...
  assert.match(script, /appendFileResultCard\(container, fileResult\);/);
  assert.match(script, /function parseFileResultLine\(text\)/);
  assert.match(script, /function appendFileResultCard\(container, fileResult\)/);
  assert.match(script, /\^\[-\*•\]\\s\+\(\.\+\)\$/);
  assert.match(css, /\.message-content\s*\{\s*[^}]*gap:\s*6px;/s);
  assert.match(css, /\.message-content\s*\{\s*[^}]*line-height:\s*1\.5;/s);
  assert.match(css, /\.md-section-label\s*\{/);
  assert.match(css, /\.md-file-card\s*\{/);
  assert.match(css, /\.md-file-path\s*\{/);
  assert.match(css, /\.md-file-detail\s*\{/);
  assert.match(css, /\.md-spacer\s*\{\s*[^}]*height:\s*5px;/s);
  assert.match(css, /\.md-heading\s*\{\s*[^}]*margin:\s*8px 0 2px;/s);
  assert.match(css, /\.md-paragraph,\s*\.md-list-item,\s*\.md-numbered-item\s*\{\s*[^}]*min-height:\s*1\.4em;/s);
  assert.match(css, /\.md-list-item,\s*\.md-numbered-item\s*\{\s*[^}]*column-gap:\s*7px;/s);
  assert.match(css, /\.md-code-block\s*\{\s*[^}]*line-height:\s*1\.45;/s);
